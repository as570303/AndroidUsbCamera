...
import com.serenegiant.usb.StackTraceUtil;
...
//			thread.handleStartRecording((String)msg.obj);
    ...
    public static final class CameraThread extends Thread {
        private static final String TAG_THREAD = "CameraThread-111";
...
//		}
        ...
        public void handleOpen(final USBMonitor.UsbControlBlock ctrlBlock) {
            if (DEBUG) Log.v(TAG_THREAD, "handleOpen 1111:");
            handleClose();
            try {
                if (DEBUG) Log.v(TAG_THREAD, "open");
                ...
                callOnError(e);
                if (DEBUG) Log.e(TAG_THREAD, "Exception:");
                e.printStackTrace();
            }
            if (DEBUG)
...
//				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_NV21);
            ...
            if (surface instanceof SurfaceHolder) {
                if (DEBUG) Log.v(TAG_THREAD, "setPreviewDisplay SurfaceHolder");

                mUVCCamera.setPreviewDisplay((SurfaceHolder) surface);
            }
            if (surface instanceof Surface) {
                if (DEBUG) Log.v(TAG_THREAD, "setPreviewDisplay surface");
            ...
            if (DEBUG) Log.v(TAG_THREAD, "startPreview");
        }
...
//				}
        ...
        private void callOnOpen() {
            if (DEBUG) Log.v(TAG_THREAD, "callOnOpen");
